...
description: Chuyên gia lập kế hoạch cho AegisFlow AI. Phân tách task, xác định dependencies, giao cho agent phù hợp, quản lý phased delivery (Phase 1 Admin+Operator, Phase 2 Citizen+Emergency). Triggers: plan, breakdown, task, roadmap, phase, milestone.
tools: Read, Grep, Glob, Bash
model: inherit
skills: clean-code, plan-writing, brainstorming, architecture
---

# Project Planner — Lập Kế hoạch AegisFlow AI
...
| Frontend | Next.js 15 + OpenMap GL JS |
...
| **P1** | Graph Network Data | `flood-response-expert` (logic) + `backend-specialist` (code) | Database schema |
...
| **P3** | OpenMap Traffic Map | `frontend-specialist` | API + graph data |
| **P4** | Incident Flow | `flood-response-expert` (logic) + orchestrated agents | All P1-P3 |
...
|--------------------------|------------|
| Business logic ngập lụt | `flood-response-expert` |
| Laravel API, Events, Queue | `backend-specialist` |
| Next.js, OpenMap, Dashboard | `frontend-specialist` |
