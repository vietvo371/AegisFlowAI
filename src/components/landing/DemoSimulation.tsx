...
import {
  ...
  Wifi, TrendingUp, MapPin, Phone, Users, Zap, Shield, Navigation
} from 'lucide-react';
...
const STEPS = [
  {
    id: 1, icon: Activity, color: 'cyan', label: 'Cảm biến', title: 'Phát hiện mực nước vượt ngưỡng',
    badge: 'LIVE', badgeColor: 'cyan',
  },
  {
    id: 2, icon: Brain, color: 'purple', label: 'AI Dự báo', title: 'Mô hình RandomForest phân tích',
    badge: '78/100', badgeColor: 'purple',
  },
  {
    id: 3, icon: AlertTriangle, color: 'red', label: 'Cảnh báo', title: 'Broadcast tức thời đến 1,240 người',
    badge: 'CRITICAL', badgeColor: 'red',
  },
  {
    id: 4, icon: Smartphone, color: 'green', label: 'Citizen SOS', title: 'Yêu cầu cứu hộ khẩn cấp',
    badge: '3 kẹt', badgeColor: 'green',
  },
  {
    id: 5, icon: Target, color: 'amber', label: 'AI Ưu tiên', title: 'Chấm điểm & xếp hạng hàng đợi',
    badge: '#1 / 87pt', badgeColor: 'amber',
  },
  {
    id: 6, icon: CheckCircle2, color: 'emerald', label: 'Điều phối', title: 'Đội cứu hộ xuất phát · ETA 8 phút',
    badge: 'DONE', badgeColor: 'emerald',
  },
];
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          { label: 'Trạm Hòa Khánh', val: '2.1 m', sub: '↑ +0.6m/h', color: 'text-red-400', icon: '🌊' },
          { label: 'Trạm Hải Châu', val: '0.4 m', sub: 'Bình thường', color: 'text-green-400', icon: '💧' },
          { label: 'Lượng mưa', val: '56 mm', sub: 'Rất lớn', color: 'text-amber-400', icon: '🌧️' },
          { label: 'Online sensors', val: '5/5', sub: 'Tất cả hoạt động', color: 'text-cyan-400', icon: '📡' },
        ].map((card, i) => (
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-3">
        <div className="flex justify-between text-[10px] text-zinc-500 mb-2">
          <span>Mực nước (m) · 30 phút qua</span>
          <span className="text-red-400 font-bold">⚠ Vượt ngưỡng</span>
        </div>
        <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{ height: 80 }}>
          ...
          <text x="4" y={h - (1.5 / maxV) * h - 3} fill="#ef4444" fontSize="8" opacity="0.8">ngưỡng 1.5m</text>
...
function AIPanel({ tick }: { tick: number }) {
  const features = [
    { label: 'Mực nước', val: 2.1, max: 3, pct: 70, color: '#a78bfa' },
    { label: 'Lượng mưa', val: 56, max: 100, pct: 56, color: '#818cf8' },
    { label: 'Thủy triều', val: 0.8, max: 2, pct: 40, color: '#6366f1' },
    { label: 'Thời gian mưa', val: 3.5, max: 10, pct: 35, color: '#4f46e5' },
  ];
    ...
    <div className="space-y-3">
      <div className="flex gap-3">
        ...
        <div className="flex flex-col items-center justify-center rounded-xl bg-white/5 border border-white/10 p-3 shrink-0">
          ...
          <div className="text-[10px] text-purple-400 font-bold mt-1">Nguy cơ CAO</div>
        </div>
        {/* feature bars */}
    ...
    <div className="space-y-3">
      ...
      <div className={cn(
        ...
        <div className="flex items-start gap-2.5">
          ...
          <div>
            <div className="text-[10px] font-extrabold text-red-400 uppercase tracking-widest">🚨 CẢNH BÁO KHẨN CẤP</div>
            <div className="text-sm font-black text-white mt-0.5">Ngập cấp độ CAO · Hòa Khánh</div>
            <div className="text-[11px] text-zinc-400 mt-1">Mực nước 2.1m · Nguy cơ 78/100 · Cư dân vùng trũng sơ tán ngay</div>
          </div>
        </div>
      </div>
      {/* broadcast stats */}
      <div className="grid grid-cols-3 gap-2">
        {[
          { icon: '📱', label: 'Push gửi', val: tick > 0.4 ? '1,240' : '...', color: 'text-cyan-400' },
          { icon: '🖥️', label: 'Dashboard', val: tick > 0.5 ? 'Live' : '...', color: 'text-green-400' },
          { icon: '📻', label: 'SMS', val: tick > 0.6 ? '340' : '...', color: 'text-amber-400' },
        ].map((s, i) => (
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-2.5 space-y-1.5">
        {[
          { t: '14:32:05', msg: 'Sensor vượt ngưỡng · Trigger AI', done: true },
          { t: '14:32:06', msg: 'AI: flood_risk = HIGH (78/100)', done: tick > 0.3 },
          { t: '14:32:07', msg: 'Alert #ALT-0001 broadcast', done: tick > 0.6 },
          { t: '14:32:08', msg: '1,240 push notifications gửi ✓', done: tick > 0.85 },
        ].map((row, i) => (
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        <div className="shrink-0 w-28 rounded-2xl border border-white/20 bg-zinc-900 overflow-hidden" style={{ aspectRatio: '9/16' }}>
          ...
          <div className="p-2 space-y-1.5">
            ...
            <div className="rounded-lg bg-red-500/20 border border-red-500/30 p-1.5 text-center">
              <div className="text-[8px] text-red-400 font-black">🆘 CỨU HỘ KHẨN</div>
            </div>
            <div className="space-y-1">
              {[
                { l: 'Địa chỉ', v: '45 NL Bằng' },
                { l: 'Người kẹt', v: '3 người' },
                { l: 'Tình trạng', v: 'Ngập 0.8m' },
              ].map((f, i) => (
            ...
            <div className={cn('rounded-lg bg-red-500 p-1.5 text-center transition-all', tick > 0.5 ? 'opacity-100' : 'opacity-40')}>
              <div className="text-[8px] text-white font-black">GỬI SOS ✓</div>
            </div>
          </div>
        </div>
        {/* request info */}
        <div className="flex-1 space-y-2">
          <div className="rounded-xl bg-white/5 border border-white/10 p-2.5">
            <div className="text-[10px] text-zinc-500 mb-1">Yêu cầu nhận được</div>
          ...
          <div className="rounded-xl bg-white/5 border border-white/10 p-2.5">
            <div className="text-[10px] text-zinc-500 mb-1">Thông tin người dùng</div>
            <div className="flex items-center gap-1.5">
              ...
              <div>
                ...
                <div className="text-[9px] text-zinc-500">Công dân đã xác thực</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-2.5">
        <div className="flex justify-between text-[10px] mb-1.5">
          <span className="text-zinc-400">Mức độ khẩn cấp</span>
          <span className="text-red-400 font-black">CRITICAL</span>
        </div>
        <div className="flex gap-1">
          {['Thấp', 'TB', 'Cao', 'Khẩn'].map((l, i) => (
...
function PriorityPanel({ tick }: { tick: number }) {
  ...
  const queue = [
    { id: 'RR-042', addr: '45 Nguyễn Lương Bằng', score: 87, urgency: 'critical', people: 3, highlight: true },
    { id: 'RR-039', addr: '12 Hoàng Diệu', score: 62, urgency: 'high', people: 1, highlight: false },
    { id: 'RR-041', addr: '88 Lê Duẩn', score: 45, urgency: 'medium', people: 2, highlight: false },
  ];
  return (
    <div className="space-y-3">
      {/* score animation */}
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 flex items-center gap-4">
        <div className="shrink-0 text-center">
          ...
          <div className="text-[10px] text-zinc-500">/100 điểm</div>
        </div>
        <div className="flex-1">
          <div className="text-xs font-black text-white mb-1">AI Priority Score</div>
          <div className="space-y-0.5">
            {[
              { label: 'Khẩn cấp critical', pts: '+30', show: tick > 0.15 },
              { label: 'Có trẻ em + người già', pts: '+25', show: tick > 0.35 },
              { label: 'Mực nước 0.8m', pts: '+18', show: tick > 0.55 },
              { label: 'Chờ 3 phút', pts: '+14', show: tick > 0.75 },
            ].map((r, i) => (
      ...
      <div className="rounded-xl bg-white/5 border border-white/10 p-2.5">
        <div className="text-[10px] text-zinc-500 mb-2 flex items-center gap-1">
          <Target size={10} /> Hàng đợi ưu tiên
        </div>
          ...
          {queue.map((q, i) => (
            <div key={i} className={cn(
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-zinc-500 text-[9px]">{q.id} · {q.people} người</div>
              </div>
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl overflow-hidden border border-white/10 relative bg-zinc-900" style={{ height: 140 }}>
        ...
        <div className="absolute bottom-2 right-2 bg-emerald-500/90 rounded-lg px-2 py-1 text-[10px] text-white font-black flex items-center gap-1">
          <Navigation size={10} /> ETA {eta} phút
        </div>
...
export function DemoSimulation() {
  ...
  useEffect(() => {
    ...
    frameRef.current = setInterval(() => {
      ...
      setProgress(t * 100);
      setTick(t);
      if (t >= 1) clearInterval(frameRef.current!);
    }, 40);
  ...
  return (
    <section className="py-28 relative overflow-hidden bg-zinc-950">
      ...
      <div className={cn('absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full blur-[120px] opacity-20 transition-all duration-1000', `bg-${step.color}-500`)} />
      ...
      <div className="container mx-auto px-4 max-w-6xl relative z-10">
        ...
        <div className="text-center mb-14">
          ...
          <h2 className="text-3xl md:text-5xl font-black tracking-tight text-white mb-4">
            Từ{' '}
            <span className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">cảm biến</span>
            {' '}đến{' '}
            <span className="bg-gradient-to-r from-amber-400 to-emerald-400 bg-clip-text text-transparent">cứu hộ</span>
            {' '}trong 4 phút
          </h2>
          <p className="text-zinc-400 text-base max-w-xl mx-auto">
            Giả lập luồng xử lý hoàn chỉnh — từ khi cảm biến phát hiện bất thường đến khi đội cứu hộ xuất phát.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-[300px_1fr] gap-6 items-start">
          ...
          <div className="space-y-1.5">
                ...
                >
                  <div className={cn(
                    ...
                    {isDone
                      ...
                      : <Icon size={16} className={isActive ? cn(sc.text, 'animate-pulse') : 'text-zinc-500'} />}
                  </div>
            ...
            <div className="rounded-xl bg-white/5 border border-white/10 p-3 mt-3 text-center">
              <div className="text-[10px] text-zinc-500">Tổng thời gian xử lý</div>
              <div className="text-xl font-black text-white">~4 phút</div>
              <div className="text-[10px] text-emerald-400 font-bold">Từ sensor → cứu hộ xuất phát</div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className={cn('flex size-10 items-center justify-center rounded-xl border', c.bg, c.border)}>
                  <step.icon size={18} className={cn(c.text, 'animate-pulse')} />
                </div>
                <div>
                  <div className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Bước {step.id} / 6</div>
                  <div className={cn('text-sm font-black', c.text)}>{step.title}</div>
                </div>
              </div>
