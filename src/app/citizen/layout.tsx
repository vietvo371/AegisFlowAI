...
import { Bell, Map, HeartPulse, User, Home, LogOut } from 'lucide-react';
...
const NAV = [
  { href: '/citizen',         icon: Home,       label: 'Tổng quan' },
  { href: '/citizen/map',     icon: Map,        label: 'Bản đồ' },
  { href: '/citizen/request', icon: HeartPulse, label: 'Cứu hộ' },
  { href: '/citizen/alerts',  icon: Bell,       label: 'Cảnh báo' },
  { href: '/citizen/profile', icon: User,       label: 'Hồ sơ' },
];
...
export default function CitizenLayout({ children }: { children: React.ReactNode }) {
  ...
  // Đang load auth — show spinner, không redirect
  if (loading) {
    return (
  ...
  // Chưa đăng nhập — useEffect sẽ redirect, render null tạm
  if (!user) return null;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      {/* Page content — padding bottom for bottom nav */}
